...
```{r}
#average annual percentage growth rate of CO2 production per country in the time range 1990-2018
owid %>% filter(!country %in% c("World", "Africa", "North America", "Europe (excl. EU-28)", "South America", "Asia","Asia (excl. China & India)", "European Union (28)", "Europe","Europe (excl. EU-27)", "European Union (27)", "High-income countries", "Upper-middle-income countries", "Lower-middle-income countries","Low-income countries", "North America (excl. USA)", "Oceania", "Antarctica"))%>% filter(year %in% c(1990, 2018))%>% select(country, year, co2, continent)%>% pivot_wider(names_from = year, values_from = co2)%>% filter(!is.na(`2018`) & !is.na(`1990`)) %>% mutate(`avg_growth_rate(%)`=((`2018`-`1990`)/`1990`)/28*100)%>%arrange(desc(`avg_growth_rate(%)`))
```

```{r}
#average annual percentage growth rate of CO2 production per capita per country in the time range 1990-2018
...
df_production_relative<-df_production_relative%>%
  mutate(`growth_rate_pcapita(%)`=(((`2018.x`/`2018.y`) - (`1990.x`/`1990.y`))/(`1990.x`/`1990.y`))/28*100)%>%
...
```{r}
#annual percentage growth rate rate per income group
owid %>% filter(country %in% c("High-income countries", "Upper-middle-income countries", "Lower-middle-income countries","Low-income countries"))%>% filter(year %in% c(1990, 2018))%>% select(country, year, co2)%>% pivot_wider(names_from = year, values_from = co2)%>%mutate(growth_rate=((`2018`-`1990`)/`1990`)/28*100)%>%arrange(desc(growth_rate))%>% filter(!is.na(`2018`) & !is.na(`1990`))
```

```{r}
#annual percentage growth rate accounting for gdp
...
```{r}
#annual annual percentage growth rate accounting for gdp per capita
...
df_production_gdp_cap<-df_production_gdp_cap%>%
  mutate(`growth_rate_gdp_pcapita(%)`=(((`2018.x`/`2018.y`) - (`1990.x`/`1990.y`))/(`1990.x`/`1990.y`))/28*100)%>%
  rename(gdpcap_18=`2018.y`)%>% rename(gdpcap_90=`1990.y`)%>%rename(emiss_18=`2018.x`)%>%rename(emiss_90=`1990.x`)%>% arrange(desc(`growth_rate_gdp_pcapita(%)`))
...
owid %>% filter(!country %in% c("World", "Africa", "North America", "Europe (excl. EU-28)", "South America", "Asia","Asia (excl. China & India)", "European Union (28)", "Europe","Europe (excl. EU-27)", "European Union (27)", "High-income countries", "Upper-middle-income countries", "Lower-middle-income countries","Low-income countries", "North America (excl. USA)", "Oceania", "Antarctica"))%>%select(country, year, consumption_co2)%>%filter(year %in% c(1990, 2018))%>% pivot_wider(names_from = year, values_from = consumption_co2)%>% 
  filter(!is.na(`2018`) & !is.na(`1990`))%>%mutate(growth_rate=((`2018`-`1990`)/`1990`)/28*100)%>%arrange(desc(growth_rate))
```
```{r}
...
df_ratio <- df_diff %>% 
  ...
  mutate(ratio_pb_cb = co2/consumption_co2)
