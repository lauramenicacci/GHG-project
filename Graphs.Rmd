...
```{r}
figure_12 <- owid %>% filter(!is.na(co2)) %>% 
  filter(!country %in% c("World", "Africa", "North America", "Europe (excl. EU-28)", "South America", "Asia", "Australia", "Asia (excl. China & India)", "European Union (28)", "Europe","Europe (excl. EU-27)", "European Union (27)", "High-income countries", "Upper-middle-income countries", "Lower-middle-income countries", "North America (excl. USA)",  "Oceania", "Antarctica")) %>%
  filter(year==2018) %>% 
...
```{r}
df_consumption_gdp%>%select(emiss_18, gdp_18, country)%>%mutate(emiss_per_gdp=emiss_18/gdp_18)%>%
...
Which states contribute to production-based emissions and consumption-based emissions the most? Are these the same states? What are the differences?
How does this change when we look at the regions? Which regions produce/consume the most?
What are the differences in share between states with the highest and lowest GDP (per capita)?


Comparing the production and consumption of the CO2 per country in absolute terms, the to 10 states that are biggest emitters in both categories are the same 10 states. The biggest emissions in terms of consumed as well as produced CO2 in 2018 was noted in China followed by US an India (fig.x). However, when we look at at the production of CO2 emissions per gdp of a country, the biggest emitters even before China are Trinidad and Tobago, Mongolia and South Africa (fig.x). When we look at the emissions from the perspective of the consumption, Luxembourg and Estonia move way up before China. Zooming out to understand more the global patterns, this paper explores the country and regional trends. Looking at the absolute values per world regions, it is aparent that the biggest emittors mainly China and the USA have major influence at the total regional emissions (fig.x)
...
new33<-df_consumption_gdp%>%mutate(conumuption_co2_gdp=emiss_18/gdp_18)%>%arrange(desc(conumuption_co2_gdp))
new33
```
